...
namespace MT_RTT
{
	...
	string extract(const string& s, const size_t& po, const size_t& pf) { return move(string(s, po, pf - po + 1)); };
	...
	vector<vector<float>> parse_arrays(const string& to_parse) {
		...
		if (to_parse.compare("null") == 0) return move(vals);
		if (to_parse.compare("NULL") == 0) return move(vals);
		if (to_parse.compare("Null") == 0) return move(vals);
		if (to_parse.compare("[[]]") == 0) return move(vals);
		if (to_parse.compare("[]") == 0) return move(vals);
		size_t P = to_parse.size() - 1;
		string temp;
		for (size_t p=1; p < P; p++) {
