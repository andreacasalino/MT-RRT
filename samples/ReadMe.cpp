...
void main() {
  // First of all, you need to explain how the particular class of problem(s)
  // you want to solve is defined. This is done by deriving an object from
  // mt_rrt::Connector. This object contains the knowledge of the problem and is
  ...
  // secondly, you need to define the object handling the sampling of states,
  // deriving something from the interface mt_rrt::Sampler. In a certain sense,
  // also this object stores a kind of knowledge of the particular problem you
  // wan to solve. Most of the time, you can use the already defined concrete
  // sampler named mt_rrt::HyperBox. Let's say your concrete sampler is called
  // MySampler
  mt_rrt::SamplerPtr my_sampler = std::make_unique<MySampler>(...);

  // group the above infomration into the structure called
  ...
  problem_description.gamma.set(
      ...); // the parameters regulating RRT* strategy, refer to the
            // documentation at Section "Compute the optimal solution: the RRT*"

  // You are now ready to build a planner.
  // This planner will absorb the problem description and will be able to solve
  // 1 or multiple problems of the same kind The easiest planner you can build
  // is mt_rrt::StandardPlanner, implementing the single threaded classic
  // strategies of the RRT. More advanced multi threaded planners are instead
  // contained in src/multi-threaded
...

  {
    ...
    std::vector<mt_rrt::State> found_sequence =
        solution.solution.value(); // access the found solution (is nullopt if
...

  {
    ...
    // passing the info of this other particular problem to solve IMPORTANT !!
    // Clearly, in order to avoid data race, you can solve only 1 problem at a
    // time per planner
    mt_rrt::State start_state = ...;
    mt_rrt::State end_state = ...;
    mt_rrt::Parameters parameters = ...;
