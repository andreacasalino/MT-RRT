...
public:
  PlanarRobotsProblemFramework(int argc, const char **argv)
      : mt_rrt::utils::SampleFramework(
            argc, argv,
            mt_rrt::samples::PlanarRobotsProblemConverter::CONVERTER) {}

protected:
  std::string defaultJsonConfig() const final { return SAMPLE_JSON; }
};

mt_rrt::State to_rad_pose(const mt_rrt::State &subject);
...
int main(int argc, const char **argv) {
  ...
  for (const auto &[start, end] : framework.problems()) {
    ...
    auto solution = planner.solve(to_rad_pose(start), to_rad_pose(end),
    ...
    log.tag = SAMPLE_NAME;
    mt_rrt::samples::PlanarRobotsProblemConverter::CONVERTER.toJson2(
        log.content, framework.problemDescription(), solution);
    log.python_visualizer = mt_rrt::utils::default_python_sources(
...
mt_rrt::State to_rad_pose(const mt_rrt::State &subject) {
  ...
  for (auto val : subject) {
    result.push_back(mt_rrt::utils::to_rad(val));
  }
  return result;
}
