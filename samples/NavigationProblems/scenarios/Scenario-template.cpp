...
public:
  NavigationProblemFramework(int argc, const char **argv)
      : mt_rrt::utils::SampleFramework(
            argc, argv,
            mt_rrt::samples::NavigationProblemConverter::CONVERTER) {}
...
int main(int argc, const char **argv) {
  ...
  for (const auto &[start, end] : framework.problems()) {
    ...
    auto solution = planner.solve(convert_state(start), convert_state(end),
    ...
    log.tag = SAMPLE_NAME;
    mt_rrt::samples::NavigationProblemConverter::CONVERTER.toJson2(
        log.content, framework.problemDescription(), solution);
    log.python_visualizer =
        mt_rrt::utils::default_python_sources(NAVIGATION_PROBLEM_PYTHON_SCRIPT);
    mt_rrt::utils::Logger::log(log);
