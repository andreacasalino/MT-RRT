...
mt_rrt::sample::Boxes2D make_scenario_obstacles();

int main(int argc, const char **argv) {
  // parse user defined inputs
  nlohmann::json user_configs;
  mt_rrt::sample::to_json(user_configs, argc, argv);

  // First of all you need to model your problem by defining a connector and
  // a sampler ... TODO
  auto static_data_problem =
      mt_rrt::sample::make_trivial_problem(make_scenario_obstacles());

  // define some default parameters
  auto parameters = mt_rrt::Parameters{
      mt_rrt::ExpansionStrategy::Star, mt_rrt::SteerIterations{10},
      mt_rrt::Iterations{3000}, mt_rrt::Determinism{0.15f}, true};
  // override the parameters according to the user specs
  mt_rrt::sample::from_json(user_configs, parameters);

  // build the kind of planner specified by the user
  auto planner = mt_rrt::sample::from_json(
      user_configs,
  ...
  mt_rrt::utils::log_scenario(
      planner->problem(), solution_01, mt_rrt::utils::POINT_CONNECTOR_LOGGER,
      mt_rrt::utils::DEFAULT_SOLUTION_LOGGER, "trivial-problem-scenario-02",
  ...
  // log results
  mt_rrt::utils::log_scenario(
      planner->problem(), solution_02, mt_rrt::utils::POINT_CONNECTOR_LOGGER,
      mt_rrt::utils::DEFAULT_SOLUTION_LOGGER, "trivial-problem-scenario-02",
      POINT_PROBLEM_SHOW_PYTHON_SCRIPT);
...
mt_rrt::sample::Boxes2D make_scenario_obstacles() {
  mt_rrt::sample::Rotator rotator{mt_rrt::sample::to_rad(-90.f)};

  std::vector<mt_rrt::sample::Boxes2D> islands_rotated;
  {
    auto &islands = islands_rotated.emplace_back();
    auto &vertical_rectangle =
        islands.emplace_back(mt_rrt::sample::Box2D{{80.f, 5.f}, {90.f, 80.f}});

    auto &horizontal_rectangle =
        islands.emplace_back(mt_rrt::sample::Box2D{vertical_rectangle});
    mt_rrt::sample::Rotator{mt_rrt::sample::to_rad(-90.f), {80.f, 80.f}}.rotate(
        horizontal_rectangle);
    mt_rrt::sample::traslate(horizontal_rectangle, {0, 10.f});

    islands.emplace_back(mt_rrt::sample::Box2D{{80.f, 80.f}, {90.f, 90.f}});

    islands.emplace_back(mt_rrt::sample::Box2D{{-35.f, 45.f}, {35.f, 55.f}});
  }
  ...
  mt_rrt::sample::Boxes2D result;
  for (const auto &island : islands_rotated) {
    result.insert(result.end(), island.begin(), island.end());
  }

  result.emplace_back(mt_rrt::sample::Box2D{{-30.f, -30.f}, {30.f, 30.f}});
  result.emplace_back(mt_rrt::sample::Box2D{{85.f, -110.f}, {110.f, -85.f}});
  result.emplace_back(mt_rrt::sample::Box2D{{-110.f, 85.f}, {-85.f, 110.f}});
