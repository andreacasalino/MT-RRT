...
class CommandEnv:
    ...
    def make():
        ...
        vars = {
            ...
            'MT_RRT_LOG_FOLDER':'${MT_RRT_LOG_FOLDER}'
        }
        frmt_to_use = 'COMMAND {}'.format(frmt[platform.system()])
        return '\n'.join(frmt_to_use.format(k, v) for k, v in vars.items())
...
class Target:
    def __init__(self, path):
        self.info = Info(path)
        self.context = {}
        context_file = os.path.join(self.info.path, 'terraform.json')
...
{CONTENT}
    ...
    def genExportBinPath_(self):
        ...
        return 'SET({TARGET}-bin-path  "{CMAKE_CURRENT_BINARY_DIR}/{TARGET}" CACHE INTERNAL "{TARGET}-bin-path")'.format(**frmt)

    def gen(self):
        with open(os.path.join(self.info.path, 'CMakeLists.txt'), 'w') as stream:
            stream.write("""###########################################################################
...
class Test(Target):
    def setUp_(self):
        ...
        for dep in ['gtest', 'gtest_main', self.info_parent.cmake_name]:
    ...
    def __init__(self, info:Info):
        Target.__init__(self, os.path.join(info.path, 'test'))
...
class Sample(Target):
    def locateScript_(self):
        ...
        for filename in os.listdir(fldr):
            if not filename.find('.py') == -1:
                return os.path.join(fldr, filename)
    ...
    def setUp_(self):
        ...
        self.context['show'] = [{
            ...
            'script':script,
            'args':"'{}'".format(os.path.join(self.info.path, filename))
        } for filename in filter(lambda name: not name.find('.json') == -1, os.listdir(self.info.path))]

    def __init__(self, info:Info):
        Target.__init__(self, os.path.join(info.path, 'samples'))
        self.info_parent = info
...
install(DIRECTORY {PATH}/header/ DESTINATION include/{NAME} FILES_MATCHING PATTERN "*.h*")""".format(**frmt)
        ...
        for name, type_name in Library.SPECIAL_FOLDERS.items():
            if os.path.exists(os.path.join(self.info.path, name)):
...
class CMake:
    ...
    def gen(self):
        with open(os.path.join(self.root, 'CMakeLists.txt'), 'w') as stream:
            for subpath in filter(lambda p: os.path.isdir(os.path.join(self.root, p)), os.listdir(self.root)):
