...
struct BenchmarkBase {
  virtual void setUp(benchmark::State &state) {
    ExtendProblem data = make_scenario(toKind(state.range(0)), toStrategy(state.range(1)));
...
struct Args {
  ...
  static const inline ParsPack standard = ParsPack{{0,1,2}, {0,1}, {100, 200, 500, 1000, 2000, 5000}};


  struct ParsPackMerge {
    ParsPackMerge() = default;
    ParsPackMerge(const ParsPack& begin) 
    : res{begin} {}
  ...
  static const inline ParsPack multiThreaded = ParsPackMerge{standard}.add({2, 3, 4, 6, 8}).res;
};

template<>
struct Benchmark<StandardPlanner>
...
BENCHMARK_TEMPLATE_F(Benchmark, StandardTest, StandardPlanner)(benchmark::State& st) {
  this->run(st);
}
BENCHMARK_REGISTER_F(Benchmark, StandardTest)->ArgsProduct(Args::standard);

template<typename PlannerT>
struct BenchmarkMultiThreaded
: public BenchmarkBase<PlannerT> {
  void setUp(benchmark::State &state) override {
...
BENCHMARK_TEMPLATE_F(Benchmark, EmbarassinglyParallelTest, EmbarassinglyParallelPlanner)(benchmark::State& st) {
  this->run(st);
}
BENCHMARK_REGISTER_F(Benchmark, EmbarassinglyParallelTest)->ArgsProduct(Args::multiThreaded);

template<>
struct Benchmark<ParallelizedQueriesPlanner>
: public benchmark::Fixture
, public BenchmarkMultiThreaded<ParallelizedQueriesPlanner> {};
BENCHMARK_TEMPLATE_F(Benchmark, ParallelizedQueriesTest, ParallelizedQueriesPlanner)(benchmark::State& st) {
  this->run(st);
}
BENCHMARK_REGISTER_F(Benchmark, ParallelizedQueriesTest)->ArgsProduct(Args::multiThreaded);

template<>
struct Benchmark<SharedTreePlanner>
: public benchmark::Fixture
, public BenchmarkMultiThreaded<SharedTreePlanner> {};
BENCHMARK_TEMPLATE_F(Benchmark, SharedTreeTest, SharedTreePlanner)(benchmark::State& st) {
  this->run(st);
}
BENCHMARK_REGISTER_F(Benchmark, SharedTreeTest)->ArgsProduct(Args::multiThreaded);

template<>
struct Benchmark<LinkedTreesPlanner>
: public benchmark::Fixture
, public BenchmarkMultiThreaded<LinkedTreesPlanner> {};
BENCHMARK_TEMPLATE_F(Benchmark, LinkedTreesTest, LinkedTreesPlanner)(benchmark::State& st) {
  this->run(st);
}
BENCHMARK_REGISTER_F(Benchmark, LinkedTreesTest)->ArgsProduct(Args::multiThreaded);
...
BENCHMARK_TEMPLATE_F(Benchmark, MultiAgentTest, MultiAgentPlanner)(benchmark::State& st) {
  this->run(st);
}
BENCHMARK_REGISTER_F(Benchmark, MultiAgentTest)->ArgsProduct(Args::multiThreaded);
