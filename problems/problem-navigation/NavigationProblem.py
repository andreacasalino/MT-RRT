...
class Cart:
    ...
    def printPose(self, pose, color = None, alpha = None):
        self.printBody_(pose, color= self.color if color == None else color, alpha=0.1 if alpha == None else alpha)
...
class Animation:
    ...
    def update_(self, pose):
        ...
        if not self.lastWaypoint == None:
            line = self.ax.plot([self.lastWaypoint[0], pose[0]], [self.lastWaypoint[1], pose[1]], c='red', linewidth=2.0)
...
class Figure(FigureBase):
    ...
    def printTree(self, tree, color):
        for index in extractTreeIndices(len(tree)):
            edge = tree[index]
            self.cart.printPose(edge['state'], alpha=0.2, color=color)
