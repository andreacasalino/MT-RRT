...
std::unique_ptr<Planner> makePlanner(ProblemDescription &&desc,
  ...
  case PlannerKind::Standard:
    res = std::make_unique<StandardPlanner>(
        std::forward<ProblemDescription>(desc));
    break;
#ifdef MT_PLANNERS_ENABLED
  case PlannerKind::Embarass:
    res = std::make_unique<EmbarassinglyParallelPlanner>(
        std::forward<ProblemDescription>(desc));
    break;
  case PlannerKind::PQuery:
    res = std::make_unique<ParallelizedQueriesPlanner>(
        std::forward<ProblemDescription>(desc));
    break;
  case PlannerKind::Shared:
    res = std::make_unique<SharedTreePlanner>(
        std::forward<ProblemDescription>(desc));
    break;
  case PlannerKind::Linked:
    res = std::make_unique<LinkedTreesPlanner>(
        std::forward<ProblemDescription>(desc));
    break;
  case PlannerKind::Multiag:
    res = std::make_unique<MultiAgentPlanner>(
        std::forward<ProblemDescription>(desc));
    break;
#endif
  }
