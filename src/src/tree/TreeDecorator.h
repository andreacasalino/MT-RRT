...
namespace mt {
	...
	public:
		...
		inline const Problem& getProblem() const override { return this->wrapped->getProblem(); };

		inline std::list<Rewird> computeRewirds(Node& pivot, const Nodes::const_reverse_iterator& delimiter) const override { return this->wrapped->computeRewirds(pivot, delimiter); };

		inline Nodes::const_reverse_iterator getDelimiter() const override { return this->wrapped->getDelimiter(); };

		inline Tree* get() { return this->wrapped.get(); };
		inline const Tree* get() const { return this->wrapped.get(); };

		template<typename TreeType>
		inline const TreeType* getT() const { return dynamic_cast<const TreeType*>(this->get()); }
		template<typename TreeType>
		inline TreeType* getT() { return dynamic_cast<TreeType*>(this->get()); } 
