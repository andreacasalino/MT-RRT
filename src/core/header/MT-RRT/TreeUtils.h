...
struct NearSetQuery {
  ...
  template <bool ComputeCost2Root> void process(Node &subject) {
    if (connector->minCost2Go(subject.state(), state_pivot) <= ray) {
      ...
      set.emplace_back(NearSetElement{&subject, cost2Root, cost2Go});
    }
  }
};
...
extend(const View &target, TreeT &tree_handler, const bool is_deterministic) {
...
template <typename TreeT>
std::optional<Connector::SteerResult>
extend_star(const View &target, TreeT &tree_handler,
            const bool is_deterministic, Rewires &rewires) {
