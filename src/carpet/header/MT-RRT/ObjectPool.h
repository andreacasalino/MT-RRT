...
template <typename T> struct IsTrivialType {
  static constexpr bool value =
      ...
      std::is_same_v<T, double> || std::is_same_v<T, std::int16_t> ||
...
template <typename T> class ObjectPool : public ObjectPoolBase<T> {
public:
...
template <> struct ObjectPool<int> : ObjectPoolMemCopiableTypes<int> {};
template <> struct ObjectPool<float> : ObjectPoolMemCopiableTypes<float> {};
template <> struct ObjectPool<double> : ObjectPoolMemCopiableTypes<double> {};
template <>
...
T &ObjectPool<T>::emplace_back(Args &&...args) {
  ...
  if (recipient->size_ == recipient->capacity_) {
    ...
    recipient =
        this->chunks_
            .emplace_back(std::make_unique<Chunk<T>>(recipient->capacity_ * 10))
            .get();
  }
...
template <typename T>
T *ObjectPoolMemCopiableTypes<T>::emplace_back(const T *source_buffer,
  ...
  if (std::size_t residual_size = recipient->capacity_ - recipient->size_;
    ...
    // create a new chunk
    recipient =
        this->chunks_.emplace_back(std::make_unique<Chunk<T>>(next_chunk_cap))
            .get();
  }
  T *slot = recipient->at(recipient->size_);
