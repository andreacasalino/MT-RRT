...
void LockFreeForwardList<T>::emplace_back(Args &&...args) {
  ...
  while (true) {
    ...
    if (cursor->next.compare_exchange_strong(
            expected, to_add, std::memory_order::memory_order_acquire)) {
...
void LockFreeForwardList<T>::forEach(Pred &&pred) {
  for (Node_ *cursor = front; cursor != nullptr;
       cursor = cursor->next.load(std::memory_order::memory_order_acquire)) {
